...
/* when clicked on "save", it hides the button and maintain the popup window
...
function Set_HomeLayout(xSource_Link)
{		
	parent.Hazard_CaseCodePopup(xSource_Link);  //once the function is activiated, the window will direct to parent (RiskFMEA_SR2.php) and run Javascript function which re-layouts the windows(close dark layer and popup window)
}
...
function Select_a_Category()
{
	...
	xmlhttp.open('get', 'RiskFMEA_SaveSameTime.php?Document='+Selected_DocNo+'&Revision='+Selected_DocRev+"&RiskID="+Selected_RiskID+"&CollumnId="+'Problem_Category'+"&CellSave="+Selected_Category);	
        ...
        {if(xmlhttp.readyState == 4)
			...
			xmlhttp.onreadystatechange = Set_Respond_Value;  //does not send to another php file without this sentence 
 			...
 			window.open("RiskFMEA_Popup.php?Category="+Selected_Category+"&Document="+Selected_DocNo+"&Revision="+Selected_DocRev+"&Cross="+"Hazard"+"&Risk_ID="+Selected_RiskID+"&CategoryChanged="+Selected_RiskID, "_self"); 	
			}
		};     
	xmlhttp.send(null);
	}
...
function Select_a_SubCategory()
{
	...
	if (xmlhttp)
	{
	xmlhttp.open('get', 'RiskFMEA_SaveSameTime.php?RiskID='+Selected_RiskID+'&CollumnId='+'Sub_Category'+'&CellSave='+Selected_SubCategory);
		
	xmlhttp.open('get', 'RiskFMEA_SaveSameTime.php?Document='+Selected_DocNo+'&Revision='+Selected_DocRev+"&RiskID="+Selected_RiskID+'&CollumnId='+'Sub_Category'+'&CellSave='+Selected_SubCategory);
        ...
        {if(xmlhttp.readyState == 4)
			...
			xmlhttp.onreadystatechange = Set_Respond_Value;  //does not send to another php file without this sentence 
 			...
 			window.open("RiskFMEA_Popup.php?Category="+Selected_Category+"&SubCategory="+Selected_SubCategory+"&Document="+Selected_DocNo+"&Revision="+Selected_DocRev+"&Cross="+"Hazard"+"&Risk_ID="+Selected_RiskID+"&SubCategoryChanged="+Selected_RiskID, "_self"); 
			}
		};     
	xmlhttp.send(null);
	}				
...
function Select_a_Hazard()
{
	...
	if (xmlhttp)
	{	
	xmlhttp.open('get', 'RiskFMEA_SaveSameTime.php?Document='+Selected_DocNo+'&Revision='+Selected_DocRev+"&RiskID="+Selected_RiskID+'&CollumnId='+'Hazard'+'&CellSave='+Selected_Hazard);		//"hazard" refers to database column
        ...
        {if(xmlhttp.readyState == 4)
			...
			xmlhttp.onreadystatechange = Set_Respond_Value;  //does not send to another php file without this sentence 
 			...
 			window.open("RiskFMEA_Popup.php?Category="+Selected_Category+"&SubCategory="+Selected_SubCategory+"&Hazard="+Selected_Hazard+"&Document="+Selected_DocNo+"&Revision="+Selected_DocRev+"&Cross="+"Hazard"+"&Risk_ID="+Selected_RiskID+"&HazardChanged="+Selected_RiskID, "_self"); 	//"hazard" refers to icon at table
			}
		};     
	xmlhttp.send(null);
	}				
...
function Select_a_HSType()
{
	...
	window.open("RiskFMEA_Popup.php?HS="+Selected_HS+"&Document="+Selected_DocNo+"&Revision="+Selected_DocRev+"&Cross="+Selected_ClickedCross+"&Risk_ID="+Selected_RiskID, "_self"); 
}
...
function Select_a_HarmType()
{
	...
	if (xmlhttp)
	{	
	xmlhttp.open('get', 'RiskFMEA_SaveSameTime.php?Document='+Selected_DocNo+'&Revision='+Selected_DocRev+"&RiskID="+Selected_RiskID+'&CollumnId='+'Harm'+'&CellSave='+Selected_Harm);		//"hazard" refers to database column
	...
	window.open("RiskFMEA_Popup.php?Harm="+Selected_Harm+"&Document="+Selected_DocNo+"&Revision="+Selected_DocRev+"&Cross="+Selected_ClickedCross+"&Risk_ID="+Selected_RiskID, "_self"); 
			}
		};     
	xmlhttp.send(null);
	}
...
function Select_a_SeverityType()
{
	...
	if (xmlhttp)
	{	
	xmlhttp.open('get', 'RiskFMEA_SaveSameTime.php?Document='+Selected_DocNo+'&Revision='+Selected_DocRev+"&RiskID="+Selected_RiskID+'&CollumnId='+'Severity'+'&CellSave='+Selected_Severity);		//"hazard" refers to database column
	...
	window.open("RiskFMEA_Popup.php?Harm="+Selected_Harm+"&Severity="+Selected_Severity+"&Document="+Selected_DocNo+"&Revision="+Selected_DocRev+"&Cross="+Selected_ClickedCross+"&Risk_ID="+Selected_RiskID, "_self"); 
			}
		};     
	xmlhttp.send(null);
	}	
...
//--------------$Display value represent data before any selection option level 1-- 
//--------------$Operation_ID is a submit indicator and its value represent data from selection ----------------- 
//----------------then has the same function as Select_ButtonOption 	
function Select_ButtonOption2($Display, $Operation_ID, $Operation, $Operation_Title, $Operation_Ary)
{	
...
$NewAry = array($Operation_ID);	//create a new array with only one variable (variable within array shall have no quoation (waste 3 hours)	
  ...
  else
  {
	...
	if (isset($Display) && $Display<>'') //if $Display=/ 0, have prefilled data from database
	{	
	...
	//-----------------------------------------------------------
	else 				//when $Display=0, else no prefilled data, but need to show title
	{	
...
echo "<script>Set_HomeLayout('RiskFMEA_SR2.php?Document=$Document_No&Revision=$Document_Revision')</script>";	//escape from child program	
	}
	else													//if button "save" not clicked, taken data from database and display on screen
	{
				
		$query_GetPartRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$query_GetRequrementRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$query_GetFMRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$query_GetFMCauseRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
if (isset($_POST['FMEAData_Submit']))		//if clicked on "save", deposit to database
	...
	$query_GetHazardRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
<!-----------------Table 6.1: Problem category (FDA lists)--------------------->
 									<!-- "DISTINCT' uses to search unique category collections --->
<?php 
				
if ($_REQUEST['CategoryChanged'] == $Risk_ID)	//1.if has a submit occurred
{
$Operation_ID = $_REQUEST['Category'];					//indicate a submit and its value as selection
$Display = $_REQUEST['Category'];						//fill the HS cell by the selection
}
else											//2. if has no submit occurred
{												    //2.2 no submit and has no problem category changes 		
		$Operation_ID = '';								//indicate no sumbit
		$Display = $Problem_Category;			       //fill the cell by data from database (if blank, show title)
}
...
Select_ButtonOption2($Display, $Operation_ID, $Operation, $Operation_Title, $Operation_Ary);	//the funtion uses to show the pull down windows	
?>
</select>
</div>	 			
</td>
		
		<!--------------Table 6.2: Sub-category (FDA lists)-------------------->		
		
<?php 
		
if ($_REQUEST['SubCategoryChanged'] == $Risk_ID)		//1.if has a submit occurred
{
$Operation_ID = $_REQUEST['SubCategory'];				//indicate a submit and its value as selection from javascript
$Display = $_REQUEST['SubCategory'];					//fill the Harm cell by the selection
}
else									//2. if has no submit occurred
{   
	if ($_REQUEST['CategoryChanged'] == $Risk_ID)
	{									//2.1 if has no submit, but have changes from previous cell "problem category"
		$Operation_ID = '';						 //indicate a submit 		
		$Display = '';							//fill the HS cell by title		
	}
	else								//2.2 if has no submit, and no changes from previous cell "problem category"
	{
		$Operation_ID = '';						// indicate no submit
		$Display = $Sub_Category;				//fill the cell by data from database
	}
}
		
$query_GetCategoryName = mysqli_query($con, "SELECT DISTINCT Sub_Category FROM Risk_FDACodes WHERE Problem_Category=\"$Problem_Category\""); //take data from database since its selection has been saved 
...
?>
		...
		<!-----------------Table 6.3: Hazards (FDA lists)--------------------->		
		
<?php 
if ($_REQUEST['HazardChanged'] == $Risk_ID)		//1.if has a submit occurred.
{
$Operation_ID = $_REQUEST['Hazard'];			//indicate a submit and its value as selection from javascript
$Display = $_REQUEST['Hazard'];					//fill the Harm cell by the selection
	
	$Hazard_Situation = "";						//very direct method rather than to create a changing token
								               //to clear the database and indicate the need for updating at "s" column
	$HS_Definition = "";
	$Harm = "";
	$Harm_Severity = "";
	$Harm_Intervention = "";
	...
	S_Pre = '$S_Pre',
	S_Post = '$S_Post',
	RiskRank_Pre = '$RiskRank_Pre',
	RiskRank_Post = '$RiskRank_Post'
...
{
	if ($_REQUEST['CategoryChanged'] == $Risk_ID || $_REQUEST['SubCategoryChanged'] == $Risk_ID)
	{				//2.1 if has no submit, but have changes from previous cell "problem category" or "sub-category"
		$Operation_ID = '';						 //indicate a submit and its value as selection from javascript		
		$Display = '';							//fill the HS cell by title		
	}
	else			//2.2 if has no submit, no changes from previous cell "problem category" and "sub-category"
	{
		$Operation_ID = '';						// indicate no submit
		$Display = $Hazard;				        //fill the cell by data from database
	}
}		
				
$query_GetHazardName = mysqli_query($con, "SELECT Hazard FROM Risk_FDACodes WHERE Problem_Category=\"$Problem_Category\" AND Sub_Category=\"$Sub_Category\" ORDER BY Hazard ASC");															
...
Select_ButtonOption2($Display, $Operation_ID, $Operation, $Operation_Title, $Operation_Ary);	
?>
		</select>
</div>	 			
</td>
...
<?php 
	if ($_REQUEST['CategoryChanged'] == $Risk_ID || $_REQUEST['SubCategoryChanged'] == $Risk_ID)
	{				//2.1 if have any changes from previous cells: "problem category", "sub-category", blank below cells
		$Hazard_Desc = '';	//blank those cells
		$FDA_Code = '';
		$NCI_Code = '';	
	}
	else if ($_REQUEST['HazardChanged'] == $Risk_ID)
	{				//2.2 if have any changes from cell "hazard", extracted "Hazard" from database, derive Hazard_Desc, codes from FDA tables
		/*					removed since the database can not be updated completely at this moment.
		$query_GetHazardRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE Hazard_ID=\"$Risk_ID\"");
		
		$HazardInfo_Row = mysqli_fetch_array($query_GetHazardRow);
		$Problem_Category = $HazardInfo_Row['Problem_Category'];
		$Sub_Category = $HazardInfo_Row['Sub_Category'];
		echo $Hazard = $HazardInfo_Row['Hazard']; */
		
		$query_GetHazardName = mysqli_query($con, "SELECT * FROM Risk_FDACodes WHERE  Hazard=\"$Hazard\"");		
		$HazardInfo_Row = mysqli_fetch_array($query_GetHazardName);
		$Hazard_Desc = $HazardInfo_Row['Hazard_Desc'];	
		$NCI_Code = $HazardInfo_Row['NCI_Code'];
		$FDA_Code = $HazardInfo_Row['FDA_Code'];	
	}
				//2.3 if has no any changes from previous cells	"problem category", "sub-category", "Hazard", use the database data to display (no change the Hazard_Desc, codes)
?>

<tr class="FMEA_PopUp_Row">	
		<td class="FMEA_PopUp_InputTitle" style="text-indent:0.7em">Hazard Description</td>    
    	<td colspan="3"><textarea rows="4" name="Hazard_Desc" style="width:97.9%; resize:none; background-color:#FFF;"><?php echo $Hazard_Desc;?></textarea></td>
</tr>		
<tr class="FMEA_PopUp_Row">	
...
<!-------------------------------When Clicked on "Harms of Failure Mode" button (8)------------------------------> 
<!---------------------------------------------------------------------------------------------------------------->
<?php
...
<?php
$query_GetHarmRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
<?php		
$query_GetSeverityRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$Harm_ID = $Severity_Row['Harm']; 							//pick up from database
				
$Operation_Ary = Table_HarmtoSeverity($Harm_ID); 	//create an array
?>
...
?>
<!-------------------------------When Clicked on "Current controls" button (9)------------------------------> 
<!---------------------------------------------------------------------------------------------------------------->
<?php
...
$query_GetCurrentControlRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");		
...
<!--------------------------When Clicked on "Recommended Mitigations" button (10)----------------------------------> 
<!---------------------------------------------------------------------------------------------------------------->
<?php
...
$query_GetMitigationRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$query_GetVerificationRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$query_GetRBARow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$query_GetNewHazardRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
$query_GetCommentRow = mysqli_query($con, "SELECT * FROM Risk_FMEA WHERE  Document_No=\"$Document_No\" AND Document_Revision=\"$Document_Revision\" AND Risk_ID=\"$Risk_ID\"");
...
<!--popup closed by main program riskFMEA.php-->
