...
function outputFMEAWorksheetToExcel($spreadsheetRows = [], $Project_ID = '', $Document_No = '', $Document_Revision = '') {
    ...
    $worksheet->setCellValue('B3', "USER MANUAL/IFU OUTPUT WORKSHEET");
    $worksheet->setCellValue('B5', "Caution or Warning");
    $worksheet->setCellValue('D5', "Hazards and Measures Description");
    $worksheet->setCellValue('Q5', "Action Results");
   

    $worksheet->setCellValue('B6', "IFU ID");
	$worksheet->setCellValue('C6', "Saftey Info");

	
    $worksheet->setCellValue('D6', "FMEA Document");
    $worksheet->setCellValue('E6', "Risk ID");
    $worksheet->setCellValue('F6', "Hazard");
    $worksheet->setCellValue('G6', "Hazardous Situations");
    $worksheet->setCellValue('H6', "Harm");
    $worksheet->setCellValue('I6', "NCI Code");
    $worksheet->setCellValue('J6', "Sub-Process Control");
    $worksheet->setCellValue('K6', "Risk Control Measure");
    $worksheet->setCellValue('L6', "Post_S");
    $worksheet->setCellValue('M6', "Post_P1");
	$worksheet->setCellValue('N6', "Post_P2");
	$worksheet->setCellValue('O6', "Post_P");
	$worksheet->setCellValue('P6', "Post_RR");
	
	$worksheet->setCellValue('Q6', "Taken");
	$worksheet->setCellValue('R6', "Comments");
...
/*
    ...
    for ($i = 5; $i <= 20; $i++) {
        ...
        $objPHPExcel->getActiveSheet()->getStyle('B' . $i . ':R' . $i)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
		
		$objPHPExcel->getActiveSheet()->getStyle('B' . $i . ':R' . $i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			
    }
	
	$objPHPExcel->getActiveSheet()->getStyle('B5:R6')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	
	$objPHPExcel->getActiveSheet()->getStyle('B5:R6')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    ...
    $objPHPExcel->getActiveSheet()->getStyle("B3:R3")->getFont()->setSize(16);

    $worksheet->getStyle('B5:R6')
            ->getFill()
    ...
    header('Content-Disposition: attachment;filename="FMEAIFUWorksheet.xlsx"');
